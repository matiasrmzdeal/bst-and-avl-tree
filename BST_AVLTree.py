...
class Tree:
    ...
    ########### ADD CODE HERE - You won't add code RIGHT HERE, but you will need to add code somewhere in the insert() method.
    ...
    def height(self):
        ans = self.height_helper(self.root)
        return ans
    ...
    def height_helper(self, node):
        ...
        leftHeight = self.height_helper(node.lChild)
        # rightHeight = self.height(node->right)
        rightHeight = self.height_helper(node.rChild)
    ...
    def range(self):
        # set currentNode to self root
        currentNode = self.root
        # check if empty tree
        if currentNode is None:
            # range is undefined
            return "undefined or maybe -1"
        # check if tree has one node
        if currentNode.lChild is None:
            # return 0
            return 0
        # get lowest value (leftmost node)
        # currentNode = self.root
        ...
        # get highest value (rightmost node)
        # reset currentNode to self.root
        currentNode = self.root
        # while rChild is not None:
        ...
        # set range
        range = maxValueNode.key - minValueNode.key
    ...
    def level(self, level):
        ...
        if self.height() < level:
            return store_nodes
        self.traverse_helper(self.root, 0, level, store_nodes)
        return store_nodes
        
    def traverse_helper(self, node, currLevel, level, store_nodes):
        # currentNode = self.root
        currentNode = node
        # if node is null
        if currentNode is None:
        ...
        if currLevel == level:
            # store_nodes.append(currentNode.key)
            store_nodes.append(currentNode)
        else:
            # traverse_helper(current.left)
            self.traverse_helper(currentNode.lChild, currLevel + 1, level, store_nodes)
            # traverse_helper(current.right)
            self.traverse_helper(currentNode.rChild, currLevel + 1, level, store_nodes)
    ...
    def left_side_view(self):
        ...
        # for level + 1 in range(levels)
        for i in range(levels+1):
            # level_nodes = []
            level_nodes = []
            # self.left_traverse_helper(self.root, currLevel, level, level_nodes)
            self.left_traverse_helper(self.root, 0, i, level_nodes)
            minNode = level_nodes[0]
            for node in level_nodes:
                if node.key < minNode.key:
                    minNode = node.key
                

            # left_side.append(level_min)
            left_side.append(minNode)
        
        # return left_side
        return left_side
    
    def left_traverse_helper(self, node, currLevel, level, level_nodes):
        ...
        # if currLevel == level:
        if currLevel == level:
            # level_nodes.append(currentNode)
            level_nodes.append(currentNode) 

        # traverse w recursion
        # else:
        else:
            # self.left_traverse_helper(currentNode.lChild, currLevel + 1, level, level_nodes)
            self.left_traverse_helper(currentNode.lChild, currLevel + 1, level, level_nodes)
            # self.left_traverse_helper(currentNode.rChild, currLevel + 1, level, level_nodes)
            self.left_traverse_helper(currentNode.rChild, currLevel + 1, level, level_nodes)
    ...
    def sum_leaf_nodes(self):
        ...
        if self.root is None:
            # return 0
            return 0
        # call traversing helper
        self.sum_leaf_helper(self.root, leaf_nodes)
        # sum_leaf_nodes = sum(leaf_nodes)
        sum_leaf_nodes = sum(leaf_nodes)
        # return sum of leaf nodes list
        return sum_leaf_nodes

    def sum_leaf_helper(self, node, leaf_nodes):
        # set currentNode to root
        currentNode = node
        if currentNode is None:
            return
        # if (currentNode.lChild is None) and (currentNode.rChild is None):
        if (currentNode.lChild is None) and (currentNode.rChild is None):
            # leaf_nodes.append(currentNode.key)
            leaf_nodes.append(currentNode.key)
            return
        # else
        else:
            # sum_leaf_helper(currentNode.lChild, leaf_nodes)
            self.sum_leaf_helper(currentNode.lChild, leaf_nodes)
            # sum_leaf_helper(currentNode.rChild, leaf_nodes)
            self.sum_leaf_helper(currentNode.rChild, leaf_nodes)
